...
public class TeleporterMiningDimension extends Teleporter {
    ...
    private BlockPositionDim pos;

    public TeleporterMiningDimension(WorldServer par1WorldServer, int destId, BlockPositionDim pos){
        ...
        this.pos = pos;
        if(pos == null) {
            pos = ModMiningDimension.instance.portalManager.getEntityPortalForId(destId);
        }
    }
    ...
    {
        if(par1Entity instanceof EntityPlayer && par1Entity.dimension == ModMiningDimension.dimensionId) {
            ((EntityPlayer) par1Entity).addChatComponentMessage(new ChatComponentText("Welcome  to the Mining Dimension"));
        }

        if(pos != null) {
            par1Entity.setLocationAndAngles((double)pos.x, (double)pos.y, (double)pos.z, par1Entity.rotationYaw, 0.0F);
        }
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlock(k1, l1, i2, flag ? Blocks.diamond_block : Blocks.air);
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
    {
        ...
        else
        {
            for (l3 = l - short1; l3 <= l + short1; ++l3)
            {
                ...
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
                {
                    ...
                    {
                        if (this.worldServerInstance.getBlock(l3, i2, l1) == Blocks.stained_glass)
                        {
                            while (this.worldServerInstance.getBlock(l3, i2 - 1, l1) == Blocks.stained_glass)
                            {
                                --i2;
                            }
        ...
        if (d3 >= 0.0D)
        {
            ...
            if (this.worldServerInstance.getBlock(i - 1, j, k) == Blocks.stained_glass)
            {
                i4 = 2;
            }
            ...
            if (this.worldServerInstance.getBlock(i, j, k - 1) == Blocks.stained_glass)
            {
                i4 = 3;
            }
        ...
        for (i3 = 0; i3 < 4; ++i3)
        {
            for (j3 = 0; j3 < 4; ++j3)
            {
                for (k3 = -1; k3 < 4; ++k3)
                {
                    ...
                    this.worldServerInstance.setBlock(l3, i4, j4, (Block)(flag ? Blocks.obsidian : Blocks.stained_glass), 0, 2);
                }
            }
